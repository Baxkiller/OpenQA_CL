...
def train(model, optimizer, scheduler, start_step, datasets, collator, opts, best_val, save_path):
    ...
    while cur_step < opts.total_steps:
        for i, batch in enumerate(train_dataloader):
            ...
            # 保证不含有正确答案
            negatives_ids = candidates_ids[:, 1::2, :]
            negatives_mask = candidates_mask[:, 1::2, :]

            can_scores, gold_scores = model(
                (negatives_ids.cuda(), negatives_mask.cuda()),  # candidates
                (answers_ids.cuda(), answers_mask.cuda()),  # answers
                (passage_ids.cuda(), passage_mask.cuda()),  # passages
            )
