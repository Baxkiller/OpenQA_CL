    ...
    while cur_step < opts.total_steps:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if cur_step % opts.eval_freq == 0:
                ...
                score, score2 = evaluate_metric(model, dataloader["eval"], tokenizer = tokenizer, opts = opts,
                ...
                logger.info(f"Evaluate at : \t{cur_step} | {opts.total_steps} \n"
                            f"Avg Loss    : \t{float(loss_sums) / opts.eval_freq: .3f}\n"
                            f"Eval score  : \t{100 * float(score) : .2f} \n"
                            f"Meteor score: \t{100 * float(score2) : .2f} \n"
        ...
        for i, batch in enumerate(eval_dataloader):
            ...
            for map_index, prediction_undecode in enumerate(predictions_undecode):
                ...
                if (map_index + 1) % n_candidate == 0:
                    ...
                    meteor_score = evaluate_metrics.meteor_group_ans(ans_group = each_question, targets = target_ans)
                    each_question = []
                    # 之前使用append(match_score=sum,现在使用avg)，但根本性质没有改变
                    # 使用min，保证有答案即为1，无答案即为0
                    all_match_score.append(min(match_score, 1))
                    all_meteor_score.append(meteor_score)

    avg_match_score = Utils.avg_value(all_match_score)
    avg_meteor_score = Utils.avg_value(all_meteor_score)
    return avg_match_score, avg_meteor_score
...
if __name__ == '__main__':
    ...
    collator = data_Util.T5_Collator(
        tokenizer = tokenizer,
